...
- `spaces/{spaceId}/todos/{todoId}/messages/{id}` — **per-todo discussion thread** (epic #247): `body` (Tiptap JSON), `text`, `tags[]`, `mentions[]`, `author` (immutable), `source` (`'user'`|`'aido'`), `sessionId` (null unless aido-authored), `createdAt`. Keeps the member↔aido back-and-forth out of the todo body. Any space member may read/post; clients may create only `source:'user'` messages (`'aido'` is written by the MCP server via the Admin SDK); no editing (`update` denied); a member deletes only their own.
...
Tools: `list-spaces`, `list-todos`, `add-todo`, `complete-todo`, `set-waiting-on`, `list-daily`, `add-daily`, `delete-todo`, `whoami`, `list-members`, the **Agent-Sessions** work loop (epic #212): `register-session`, `next-todo`, `update-todo`, `handoff`, and the per-todo **discussion thread** (epic #247): `post-message`, `list-messages`.

**Agent-Sessions (epic #212) — let a Claude-Code session work off todos.** `register-session(spaceId, hostname, workingFolder)` (call first) upserts a space-bound session and returns a deterministic `sessionId`. In a `/loop`: `next-todo` **claims** (transaction + lease) the oldest open todo bound to that session and returns its body **and its discussion thread** as Markdown; `update-todo` writes the **result** into the body (Markdown→Tiptap incl. code blocks, **`replace` by default** — the body holds the current result, not a growing Q&A — `src/lib/tiptap/markdown.ts`, dep `markdown-it`); `handoff` returns it **open** to the human and **releases the session binding** (re-attach to have the agent work it again); `complete-todo` (with `sessionId`) closes it — both clear `attachedSession`/claim so a done/handed-off todo is no longer "assigned" in the UI. **The conversation stays in the thread, not the body (epic #247):** `post-message` posts a question/note into the claimed todo's thread (`spaces/{spaceId}/todos/{todoId}/messages`), so a typical rework loop is *post-message (question) → handoff → human answers in the thread → next-todo re-reads it*; `list-messages` reads a thread on demand. Wirkung is bounded by **scope-to-claimed-todo** + a **per-session tool allowlist** (default `['update-todo','handoff','post-message']`). The user attaches todos to a session in the web UI (`AttachToSessionMenu`, status badge via `StatusBadge`) and manages sessions in settings (`AgentSessionsSettings`). **No new env var** — Agent-Sessions use the personal API key.
